...
from marilib.model import EdgeEvent, GatewayInfo, MariGateway, MariNode, SCHEDULES
...
class MariLib:
    ...
    def __post_init__(self):
        if self.port is None: self.port = get_default_port()
    ...
    def on_data_received(self, data: bytes):
        ...
        if event_type == EdgeEvent.NODE_JOINED:
            ...
            if n := self.gateway.remove_node(int.from_bytes(data[1:9], "little")): self.cb_application(EdgeEvent.NODE_LEFT, n)
            ...
            try: self.gateway.set_info(GatewayInfo().from_bytes(data[1:]))
