...
class LoadTester(threading.Thread):
    ...
    def __init__(
        self, mari: MariLib, schedule_id: int, load: int,
    ...
    def run(self):
        ...
        if max_rate == 0:
            sys.stderr.write(
                f"Error: Invalid schedule_id '{self.schedule_id}'.\n"
            )
            return
        ...
        while not self._stop_event.is_set():
            ...
            if nodes_exist:
                self.mari.send_frame(
                    MARI_BROADCAST_ADDRESS, LOAD_PACKET_PAYLOAD
                )
...
@click.command()
@click.option(
    "--port", "-p", type=str, default=SERIAL_PORT_DEFAULT,
    show_default=True, help="Serial port to use (e.g., /dev/ttyACM0)",
)
@click.option(
    "--schedule",
...
@click.option(
    "--load", type=int, default=0, show_default=True,
