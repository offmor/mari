...
class LoadTester(threading.Thread):
    ...
    def __init__(self, mari: MariLib, schedule_id: int, load: int, stop_event: threading.Event):
...
def main(port: str | None, schedule: str, load: int):
    ...
    try:
        # Main loop for "normal" application data (statistically counted)
        ...
        while not stop_event.is_set():
            ...
            # Send normal traffic periodically
        ...
        stop_event.set()
        if latency_tester: latency_tester.stop()
        if load_tester.is_alive(): load_tester.join()
        tui.close()
